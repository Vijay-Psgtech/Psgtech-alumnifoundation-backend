...
// ✅ FIXED: Initialize Stripe only if STRIPE_SECRET_KEY exists
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!isAnonymous && (!donorName || !donorEmail)) {
      return res
        .status(400)
        .json({ message: "Name and email required for non-anonymous donations" });
    }
    ...
    if (currency === "INR" && paymentMethod !== "Cheque" && paymentMethod !== "Wire Transfer") {
    ...
    } else if (paymentMethod === "Cheque" || paymentMethod === "Wire Transfer") {
...
// ─────────────────────────────────────────────────────────────
// 5️⃣ VERIFY STRIPE PAYMENT
// ─────────────────────────────────────────────────────────────
// POST /api/donations/verify-stripe
router.post("/verify-stripe", async (req, res) => {
  try {
    ...
    if (session.payment_status !== "paid") {
      return res
        .status(400)
        .json({ message: "Payment not completed", status: session.payment_status });
    }
...
// ─────────────────────────────────────────────────────────────
// 6️⃣ GET DONOR'S DONATIONS (Protected Route)
// ─────────────────────────────────────────────────────────────
  ...
  try {
    ...
    const stats = {
      ...
      totalAmount: donations.reduce((sum, d) => {
        const factor = d.currency === "USD" ? 83 : 1; // Rough USD to INR conversion
        return sum + d.amount * factor;
      }, 0),
      averageDonation:
        donations.length > 0
          ? Math.round(
...
// ─────────────────────────────────────────────────────────────
// 7️⃣ GET ALL DONATIONS (Admin Only)
// ─────────────────────────────────────────────────────────────
...
async function sendDonationConfirmationEmail(donation) {
  ...
  // For now, just log it
  console.log(`📧 Sending confirmation email to ${donation.donorEmail}`);
